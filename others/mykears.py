...
def mean_abs_error(output, target):
    ...
    target : tensorflow variable
        A distribution with shape: [batch_size, n_feature].
    """
    with tf.name_scope("mean_abs_error_loss"):
	mse = tf.reduce_mean(tf.abs(tf.sub(tf.exp(output), tf.exp(target))))
	return mse
...
def nn_model():
    ...
    model.compile(loss=mean_abs_error, optimizer='adadelta')
    return(model)
...
df.to_csv('../result/preds_oob_my_keras.csv', index=False)

# test predictions
pred_test /= (nfolds*nbags)
df = pd.DataFrame({'id': id_test, 'loss': pred_test})
df.to_csv('../result/submission_my_keras.csv', index=False)
